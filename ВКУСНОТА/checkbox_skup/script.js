(function(window){
	...
	//	this.checkedTreeItems = []; //all checkedItems
	...
	window.JCCatalogElement.prototype = {
			...
			if (this.errorCode === 0)
			{
				...
				switch (this.productType)
				{
					...
					//	treeItems = this.obTree.querySelectorAll('li');
						treeItems = this.obTree.querySelectorAll('input[type=checkbox]');


						//treeItems = BX.findChildren(this.obTree, {tagName: ['li', 'input']}, true);
		...
		selectOfferProp: function()
		{
			var i = 0,
				...
				checkedTreeItems = [...this.obTree.querySelectorAll('input:checked')];
				console.log(checkedTreeItems);
			...
			if(target.checked){
				if	(target && target.hasAttribute('data-treevalue') ) {
							...
							if (typeof document.activeElement === 'object')
								...
								rowItems = BX.findChildren(target.parentNode, {tagName: 'input'}, false);
								console.log(rowItems);
									if (checkedTreeItems && checkedTreeItems.length)
										{
										for (i = 0; i < checkedTreeItems.length; i++)
											{
											strTreeValue.push(checkedTreeItems[i].getAttribute('data-treevalue'));
												for(j = 0; j < strTreeValue.length; j++)
												{
													arTreeItem = strTreeValue[j].split('_');
										//			arTreeItem = strTreeValue.split('_');
													this.searchOfferPropIndex(arTreeItem[0], arTreeItem[1]);
											...
											if (this.smallCardNodes.panel)
												{
												smallCardItem = this.smallCardNodes.panel.querySelector('[data-treevalue="' + strTreeValue[j] + '"]');
												if (smallCardItem)
													{
			...
			//	if	(target && target.hasAttribute('data-treevalue') ) {
								strTreeValue = target.getAttribute('data-treevalue');
								arTreeItem = strTreeValue.split('_');
								this.searchOfferPropIndex(arTreeItem[0], null);
		...
		searchOfferPropIndex: function(strPropID, strPropValue)
		{
			...
			if (index > -1)
			{
				...
				//checkItem = target.getAttribute('checkbox');
				//	console.log(this);
				//if (checkItem) {
			...
			//	console.log(rowItems);
				...
				//	console.log(value);
					...
					if (isCurrent)
					{
						...
						//rowItems[i].setAttribute('checkbox',true);
					}
					else
					{
						BX.removeClass(rowItems[i], 'selected');
						//rowItems[i].setAttribute('checkbox',false);
					}
